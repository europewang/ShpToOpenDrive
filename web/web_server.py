...
class Web3DServer:
    ...
    def _convert_shp_to_xodr_with_main(self, shp_path, output_path, road_width=3.5, lane_count=2):
        ...
        try:
            ...
            if success:
                print(f"成功使用main.py转换逻辑导出XODR文件: {output_path}")
                return True
            else:
                print("使用main.py转换逻辑失败")
                return False
                
        except Exception as e:
            print(f"调用main.py转换逻辑时出错: {e}")
            return False
...
def convert_xodr_to_obj():
    ...
    try:
        ...
        # 转换XODR到OBJ
        success = web3d_server.xodr_to_obj_converter.convert(xodr_path, obj_path)
