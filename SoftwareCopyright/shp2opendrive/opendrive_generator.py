...
try:
    ...
    class ConversionHandler(ABC):
        ...
        def _handle_next(self, request: Dict[str, Any]) -> Dict[str, Any]:
            ...
            return request
class OpenDriveGenerationHandler(ConversionHandler):
    def __init__(self, name: str = "ConvertedRoad", curve_fitting_mode: str = "polyline", 
        ...
        super().__init__()
        self.opendrive_generator = OpenDriveGenerator(name, curve_fitting_mode, polynomial_degree, curve_smoothness)
    def handle(self, request: Dict[str, Any]) -> Dict[str, Any]:
        if request.get('stage') != 'opendrive_generation':
            return self._handle_next(request)
        converted_roads = request.get('converted_roads', [])
        ...
        for road_data in converted_roads:
            if road_data.get('type') == 'lane_based':
                road_id = self.opendrive_generator.create_road_from_lane_surfaces(
                    road_data.get('lane_surfaces', []), 
                    road_data.get('attributes', {})
                )
            else:
                road_id = self.opendrive_generator.create_road_from_segments(
                    road_data.get('segments', []), 
                    road_data.get('attributes', {})
                )
            if road_id > 0:
                road_ids.append(road_id)
        if road_ids and output_path:
            success = self.opendrive_generator.generate_file(output_path)
            request['success'] = success
            request['road_ids'] = road_ids
        else:
        ...
        for i, surface in enumerate(lane_surfaces):
            ...
            if width_profile:
                ...
                if min_width > 0:
                    ...
                    width_change_ratio = width_change / avg_width if avg_width > 0 else float('inf')
                if width_change > 0.1 and width_change_ratio > 0.03:
                    ...
                    max_segments = min(8, len(filtered_profile))
